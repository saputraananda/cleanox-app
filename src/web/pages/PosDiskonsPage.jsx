...
function formatDiscountValue(type, value) {
  if (type === 'additional') return 'Input di transaksi';
  if (type === 'persen') return `${Number(value || 0)}%`;
  return `Rp ${Number(value || 0).toLocaleString('id-ID')}`;
}

export default function PosDiskonsPage() {
  ...
  const loadData = async () => {
    ...
    setError('');
    try {
      const { data } = await api.get('/pos-master/discounts', {
        params: {
      ...
      setError(err.response?.data?.message || 'Gagal memuat diskon');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = async (row) => {
    setError('');
    try {
      const { data } = await api.get(`/pos-master/discounts/${row.id}`);
      const discount = data.discount;
      setEditingId(discount.id);
      setForm({
        name: discount.name || '',
        discount_type: discount.discount_type || 'nominal',
        discount_value: String(discount.discount_value ?? ''),
        description: discount.description || '',
        status: discount.status || 'Aktif',
      });
      setPanelOpen(true);
    } catch (err) {
      setError(err.response?.data?.message || 'Gagal memuat detail diskon');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await api.put(`/pos-master/discounts/${editingId}`, payload);
      } else {
        await api.post('/pos-master/discounts', payload);
      }
      setPanelOpen(false);
      setForm(emptyForm);
      setEditingId(null);
  ...
  const pagination = useMemo(
    () => paginateList(discounts, currentPage, pageSize),
    [discounts, currentPage, pageSize]
  );
  ...
  return (
    <div className="p-3 sm:p-5 space-y-6 max-w-[1400px] mx-auto bg-slate-50 min-h-full">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-[13px] text-slate-500">
            Master diskon tambahan transaksi (terpisah dari promo).
          </p>
        </div>
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-0 shadow-sm overflow-hidden">
        ...
        {loading ? (
          <div className="py-14 text-center text-[13px] text-slate-500">Memuat diskon...</div>
        ) : (
                ...
                {discounts.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-4 py-16 text-center">
                      ...
                      <p className="text-gray-300 text-xs mt-1">Coba ubah kata kunci atau filter status diskon</p>
                    </td>
                  </tr>
                ) : (
                  pagination.items.map((row) => (
                    <tr key={row.id} className="border-b border-gray-50 hover:bg-slate-50/40 transition-colors even:bg-slate-50/20">
                      ...
                      <td className="px-3 sm:px-4 py-2.5 text-sm text-gray-700 capitalize">
                        {row.discount_type === 'additional'
                          ? 'additional'
                          : row.discount_type === 'persen'
                            ? 'persen (fixed)'
                            : 'nominal (fixed)'}
                      </td>
      ...
      {panelOpen && (
        <BodyPortal>
          <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
            <div className="w-full max-w-xl max-h-[90vh] overflow-y-auto rounded-[20px] border border-slate-200 bg-white p-5 shadow-xl space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-[16px] font-bold tracking-[-0.01em] text-slate-900">
                  {editingId ? 'Edit Diskon' : 'Tambah Diskon'}
                </h2>
                <button
                  type="button"
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                <label className="block space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="Contoh: Pembulatan 500"
                  />
                </label>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setForm((prev) => ({
                          ...
                          discount_value:
                            e.target.value === 'additional' ? '' : prev.discount_value,
                        }))
                      }
                      className={inputClass}
                    >
                      <option value="nominal">Nominal (fixed)</option>
                      <option value="persen">Persen (fixed)</option>
                      <option value="additional">Additional</option>
                    </select>
                  </label>
                  {form.discount_type === 'additional' ? (
                    <div className="block space-y-1.5">
                      <span className="text-[9.5px] font-semibold uppercase tracking-[.14em] text-slate-400">
                        Nilai
                      </span>
                      <p className="rounded-[12px] border border-dashed border-slate-200 bg-slate-50 px-3 py-2.5 text-[13px] text-slate-500">
                        Nominal diisi saat transaksi
                      </p>
                    </div>
                  ) : (
                    <label className="block space-y-1.5">
                      ...
                      <input
                        ...
                        className={inputClass}
                        placeholder="Contoh: 500 atau 10"
                      />
                    </label>
                  )}
                </div>
                ...
                {form.discount_type !== 'additional' && (
                  <p className="text-[11.5px] text-slate-500">
                    Preview: {formatDiscountValue(form.discount_type, form.discount_value || 0)}
                  </p>
                )}
                ...
                <label className="block space-y-1.5">
                  ...
                  <textarea
                    ...
                    className={inputClass}
                    placeholder="Contoh: Additional untuk pembulatan kecil"
                  />
                </label>
