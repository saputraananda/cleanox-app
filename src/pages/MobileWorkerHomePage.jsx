...
import { Camera, ClipboardList, Sparkles, ArrowRight, Clock3, XCircle, CalendarDays } from 'lucide-react';
...
function getActivityIconClass(type, status) {
  ...
  if (type === 'absensi') return 'bg-[#EEF8E3] text-[#163A22]';
...
function buildTodayActivityCards({ attendance, assignedTasks, todayTasks, kebersihan }) {
  ...
  if (!attendanceRow?.check_in_at) {
    absensiCard = {
      ...
      description: 'Lengkapi check-in dan 4 foto QC untuk memulai hari kerja.',
      to: '/mobile-worker/attendance',
    };
    ...
    absensiCard = {
      ...
      description: 'Check-in dan check-out sudah tercatat.',
      to: '/mobile-worker/attendance',
    };
  }

  // Assigned = semua tanggal (perlu accept); In_Schedule / On_Progress = hari ini saja
  const assignedList = Array.isArray(assignedTasks) ? assignedTasks : [];
  const todayList = Array.isArray(todayTasks) ? todayTasks : [];
  const assignedCount = assignedList.filter((row) => row.assignment_status === 'Assigned').length;
  const inScheduleCount = todayList.filter((row) => row.assignment_status === 'In_Schedule').length;
  const onProgressCount = todayList.filter((row) => row.assignment_status === 'On_Progress').length;
  let taskCard = null;
  if (assignedCount > 0) {
    taskCard = {
      id: 'task-konfirmasi',
      ...
      description:
        assignedCount > 1
          ? `${assignedCount} tugas On Review menunggu konfirmasi`
          : '1 tugas On Review menunggu konfirmasi',
      to: '/mobile-worker/tasks',
    };
    ...
    taskCard = {
      ...
      description:
        onProgressCount > 1
          ? `${onProgressCount} tugas On Progress — selesaikan di mobile`
          : '1 tugas On Progress — selesaikan di mobile',
      to: '/mobile-worker/tasks',
    };
  } else if (inScheduleCount > 0) {
    taskCard = {
      id: 'task-schedule',
      type: 'task',
      status: 'schedule',
      ...
      description:
        inScheduleCount > 1
          ? `${inScheduleCount} tugas In Schedule — klik Mulai`
          : '1 tugas In Schedule — klik Mulai',
      to: '/mobile-worker/tasks',
    };
  }
  // Tidak ada Assigned (lintas tanggal) / In_Schedule|On_Progress hari ini → kartu Task tidak ditampilkan

  let kebersihanCard;
  if (kebersihan?.status === 'Completed') {
    kebersihanCard = {
      id: 'kebersihan-selesai',
      type: 'kebersihan',
      status: 'selesai',
      title: 'Pengecekan kebersihan sudah dilaksanakan',
      description: 'Checklist kebersihan hari ini sudah selesai.',
      to: '/mobile-worker/kebersihan',
    };
  } else {
    const uploaded = Number(kebersihan?.uploaded_count || 0);
    const required = Number(kebersihan?.required_count || 4);
    kebersihanCard = {
      id: 'kebersihan-belum',
      type: 'kebersihan',
      status: 'belum',
      title: 'Anda belum melakukan pengecekan kebersihan',
      description:
        kebersihan?.status === 'In_Progress'
          ? `Progress foto area ${uploaded}/${required}. Lengkapi sampai 4 foto.`
          : 'Lengkapi 4 foto area hari ini.',
      to: '/mobile-worker/kebersihan',
    };
  }

  return [absensiCard, taskCard, kebersihanCard].filter(Boolean);
}
  ...
  useEffect(() => {
    ...
    const fetchActivityOnce = () =>
      Promise.allSettled([
        ...
        api.get('/mobile-kebersihan/today-status'),
      ...
      try {
        ...
        const todayTasks =
          todayTasksSettled.status === 'fulfilled'
            ...
            : [];
        const kebersihan =
          kebersihanSettled.status === 'fulfilled' ? kebersihanSettled.value.data : null;
        const notices =
        ...
        if (kebersihanSettled.status === 'rejected') {
          errors.push(kebersihanSettled.reason?.response?.data?.message || 'Gagal memuat kebersihan');
        }

        setActivityCards([
          ...mapNoticesToCards(notices),
          ...buildTodayActivityCards({ attendance, assignedTasks, todayTasks, kebersihan }),
        ]);
              ...
              {MENU_ITEMS.map((item) => {
                ...
                const content = (
                  <div className="flex flex-col items-center text-center">
                    <div
                      className={`w-[58px] h-[58px] rounded-[18px] grid place-items-center shadow-[0_10px_20px_rgba(15,23,42,.08)] border border-slate-100 ${
                        item.title === 'Absensi'
                ...
                return item.active ? (
                  ...
                  <div key={item.title} className="opacity-85">
                    {content}
                  </div>
                );
              })}
              ...
              activityCards.map((item) => {
                ...
                return (
                  <div
                    ...
                    onClick={isNotice ? () => handleNoticeOpen(item) : undefined}
                    onKeyDown={
                      isNotice
                        ? (e) => {
                    ...
                    className={`rounded-[22px] border border-slate-100 bg-white p-4 shadow-[0_10px_28px_rgba(15,23,42,.05)] ${
                      isNotice ? 'cursor-pointer active:scale-[.99] transition' : ''
                    }`}
                  >
